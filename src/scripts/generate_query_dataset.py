...
def generate_sample_queries():
    queries = set()
    for prompt, (rank, taxon, phrase, data_type, timeframe) in itertools.product(
        prompts, itertools.product(ranks, taxa, phrases, data_types, timeframes)
    ):
        query = prompt.format(
            rank=rank,
            taxon=taxon,
...
def generate_json_output(query):
    """
    Generate JSON output based on the given query.

    Parameters:
    - query (str): The input query string.

    Returns:
    - output (dict): The generated JSON output containing the identified entities
      and intent.
    ...
    output = {
        key: next((item for item in items if item in query), None)
        for key, items in lookup_tables.items()
        if next((item for item in items if item in query), None)
    }

    # Determine the intent
    if query.lower().startswith("how many"):
        output["intent"] = "count"
    elif query.lower().startswith("list all") or query.lower().startswith(
        "find samples"
    ):
    ...
    time_frame_to_query = {
        "this month": "last_updated>={}".format(
            datetime.now().replace(day=1).strftime("%Y-%m-%d")
        ),
        "recently": "last_updated>=recent",
    }
...
def construct_url(json_output):
    """
    Constructs a URL based on the provided JSON output.

    Parameters:
        json_output (dict): A dictionary containing the JSON output.
...
for query in sample_queries:
    ...
    output_list.append(
        {"english_query": query, "json_output": json_output, "api_query": url}
    )
